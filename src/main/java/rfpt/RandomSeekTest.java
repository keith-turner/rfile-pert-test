...
public class RandomSeekTest {
  ...
  private static void print(CacheStats stats) {
    System.out.println(stats.getRequestCount()+" "+stats.getHitCount()+" "+stats.getMissCount());
  ...
  private static void randomseeks(Configuration conf, FileSystem fs, AccumuloConfiguration acuconf,
    ...
    sw.start();
    CachableBlockFile.Reader _cbr = new CachableBlockFile.Reader(fs, new Path(file), conf, dataCache, indexCache, acuconf);
